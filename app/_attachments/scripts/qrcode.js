...
qrcode.decode = function(src){
	...
	if(arguments.length==0){
		...
		image.onload=function(){
			...
			var context = canvas_qr.getContext('2d');
			canvas_qr.width = image.width/div;
			canvas_qr.height = image.height/div;
            context.drawImage(image, 0, 0, image.width/div, image.height/div);
			qrcode.width = image.width/div;
			qrcode.height = image.height/div;
			try{
				qrcode.imagedata = context.getImageData(0, 0, image.width/div, image.height/div);
			}
			/*** MODIFICATION ***/
			catch(e){
